    ...
    <>
      <div className="md:hidden fixed bottom-0 left-0 w-full glass border-t border-border z-50">
        <nav className="flex items-center justify-around p-3">
          <Link to="/" className={cn("flex flex-col items-center gap-1", pathname === "/" ? "text-primary" : "text-foreground/70")}>
            <LayoutDashboard className="w-5 h-5" />
            <span className="text-[10px] font-medium">대시보드</span>
          </Link>
          {isPM && (
            <Link to="/members" className={cn("flex flex-col items-center gap-1", pathname === "/members" ? "text-primary" : "text-foreground/70")}>
          ...
          <Link to="/approvals" className={cn("flex flex-col items-center gap-1", pathname === "/approvals" ? "text-primary" : "text-foreground/70")}>
        ...
        <div className="md:hidden fixed inset-0 z-[100] flex items-end">
          ...
          <div className="relative w-full bg-background border-t border-border rounded-t-2xl p-4 pb-8 animate-in slide-in-from-bottom duration-200">
            ...
            <div className="border-t border-border pt-2 flex flex-col gap-1">
              <Link to="/profile" onClick={() => setMenuOpen(false)} className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold hover:bg-black/5 dark:hover:bg-white/5">
                <UserIcon className="w-4 h-4" /> 프로필
              </Link>
              <Link to="/settings" onClick={() => setMenuOpen(false)} className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold hover:bg-black/5 dark:hover:bg-white/5">
