        ...
        <div className="lg:col-span-2">
          <div className="glass p-6 rounded-xl h-full">
            <div className="flex justify-between items-center mb-5">
              ...
              <Link to="/tasks" className="flex items-center gap-1 text-xs text-primary font-semibold hover:underline transition-colors">전체보기 <ArrowUpRight className="w-3.5 h-3.5" /></Link>
            </div>
            <div className="space-y-3">
              {activityLog.length === 0 ? (
                ...
                activityLog.slice(0, 5).map((log: ActivityLog, i: number) => (
                  <Link key={i} to={`/projects/${log.projectId}`} className="flex items-start gap-3 pb-3 border-b border-foreground/5 last:border-0 last:pb-0 group">
              ...
              <>
                ...
                <div className="space-y-2 mt-2">
                  {statusChart.map((d: any) => (
                    <Link
                      key={d.name}
                      to={`/tasks?status=${STATUS_CODE_BY_LABEL[d.name] ?? ""}`}
          ...
          <div className="glass p-6 rounded-xl h-full">
            ...
            {projectList.length === 0 ? (
              <div className="py-10 text-center text-muted-foreground space-y-2">
                ...
                {isPM && (
                  <Link to="/project/new" className="inline-block mt-2 px-4 py-2 bg-primary text-white rounded-lg text-sm font-bold">
                    첫 프로젝트 만들기
                  </Link>
                )}
              </div>
